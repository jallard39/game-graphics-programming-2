	...
	DemoLightingOptions& lightOptions)
{
	...
	ImGui::Begin("Inspector");
	{
		...
		if (ImGui::TreeNode("Particle Emitters"))
		{
			...
			{
				ImGui::PushID(emitters[i].get());
				if (ImGui::TreeNode("Emitter Node", "Emitter %d", i))
				{
...
// --------------------------------------------------------
// Builds the UI for a single particle emitter
// --------------------------------------------------------
void UIEmitter(std::shared_ptr<Emitter> emitter)
{
	...
	// Transform details
	std::shared_ptr<Transform> trans = emitter->GetTransform();
	XMFLOAT3 pos = trans->GetPosition();
	XMFLOAT3 rot = trans->GetPitchYawRoll();
	XMFLOAT3 sca = trans->GetScale();
	...
	ImGui::Text("Index First Alive: %d", emitter->GetIndexFirstAlive());
	ImGui::Text("Index First Dead: %d", emitter->GetIndexFirstDead());
	ImGui::Text("Number of Living Particles: %d", emitter->GetNumLivingParticles());
	ImGui::Text("Time Since Last Emit: %d", emitter->GetTimeSinceLastEmit());

	// Texture
	D3D11_SHADER_RESOURCE_VIEW_DESC desc = {};
	emitter->GetParticleTexture()->GetDesc(&desc);
	if (desc.ViewDimension == D3D11_SRV_DIMENSION_TEXTURE2D) 
		ImGui::Image(emitter->GetParticleTexture().Get(), ImVec2(256, 256));
