...
void Game::LoadAssetsAndCreateEntities()
{
	...
	// Depth state for refraction silhouettes
	D3D11_DEPTH_STENCIL_DESC depthDesc = {};
	depthDesc.DepthEnable = true;
	depthDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO; // No depth writing
	depthDesc.DepthFunc = D3D11_COMPARISON_LESS;
	Graphics::Device->CreateDepthStencilState(&depthDesc, silhouetteDepthState.GetAddressOf());
...
#undef LoadTexture
	...
	meshes.insert(meshes.end(), { cubeMesh, sphereMesh, torusMesh });
	...
	std::shared_ptr<Material> refractiveMat1 = std::make_shared<Material>("Refractive", refractionPS, vertexShader, XMFLOAT3(1, 1, 1), XMFLOAT2(2, 2), XMFLOAT2(1, 1), true, 0.25f);
	refractiveMat1->AddSampler("BasicSampler", sampler);
	refractiveMat1->AddSampler("ClampSampler", clampSampler);
	refractiveMat1->AddTextureSRV("NormalMap", bronzeN);

	std::shared_ptr<Material> refractiveMat2 = std::make_shared<Material>("Refractive", refractionPS, vertexShader, XMFLOAT3(1, 0.5, 0.5), XMFLOAT2(2, 2), XMFLOAT2(1, 1), true, 0.75f);
	refractiveMat2->AddSampler("BasicSampler", sampler);
	refractiveMat2->AddSampler("ClampSampler", clampSampler);
	refractiveMat2->AddTextureSRV("NormalMap", bronzeN);

	std::shared_ptr<Material> refractiveMat3 = std::make_shared<Material>("Refractive", refractionPS, vertexShader, XMFLOAT3(1, 1, 0.3), XMFLOAT2(2, 2), XMFLOAT2(1, 1), true, 0.1f);
	refractiveMat3->AddSampler("BasicSampler", sampler);
	refractiveMat3->AddSampler("ClampSampler", clampSampler);
	refractiveMat3->AddTextureSRV("NormalMap", bronzeN);

	std::shared_ptr<Material> refractiveMat4 = std::make_shared<Material>("Refractive", refractionPS, vertexShader, XMFLOAT3(0.5, 0.5, 1), XMFLOAT2(2, 2), XMFLOAT2(1, 1), true, 0.1f);
	refractiveMat4->AddSampler("BasicSampler", sampler);
	refractiveMat4->AddSampler("ClampSampler", clampSampler);
	refractiveMat4->AddTextureSRV("NormalMap", bronzeN);

	// Add materials to list
	materials.insert(materials.end(), { cobbleMat2x, cobbleMat4x, floorMat, paintMat, scratchedMat, bronzeMat, roughMat, woodMat, refractiveMat1, refractiveMat2, refractiveMat3, refractiveMat4 });
	...
	cobSphere->GetTransform()->SetPosition(-6, 0, 2);

	std::shared_ptr<GameEntity> floorSphere = std::make_shared<GameEntity>(sphereMesh, floorMat);
	floorSphere->GetTransform()->SetPosition(-4, 0, 2);

	std::shared_ptr<GameEntity> paintSphere = std::make_shared<GameEntity>(sphereMesh, paintMat);
	paintSphere->GetTransform()->SetPosition(-2, 0, 2);

	std::shared_ptr<GameEntity> scratchSphere = std::make_shared<GameEntity>(sphereMesh, scratchedMat);
	scratchSphere->GetTransform()->SetPosition(0, 0, -2);

	std::shared_ptr<GameEntity> bronzeSphere = std::make_shared<GameEntity>(sphereMesh, bronzeMat);
	bronzeSphere->GetTransform()->SetPosition(2, 0, -2);

	std::shared_ptr<GameEntity> roughSphere = std::make_shared<GameEntity>(sphereMesh, roughMat);
	roughSphere->GetTransform()->SetPosition(4, 0, -2);

	std::shared_ptr<GameEntity> woodSphere = std::make_shared<GameEntity>(sphereMesh, woodMat);
	woodSphere->GetTransform()->SetPosition(6, 0, -2);

	// Refractive entities

	std::shared_ptr<GameEntity> glassWall = std::make_shared<GameEntity>(cubeMesh, refractiveMat1);
	glassWall->GetTransform()->SetPosition(0, 0, 0);
	glassWall->GetTransform()->SetScale(4, 2, 0.2);

	std::shared_ptr<GameEntity> glassSphere = std::make_shared<GameEntity>(sphereMesh, refractiveMat1);
	glassSphere->GetTransform()->SetPosition(3, 1, -4);

	std::shared_ptr<GameEntity> glassSphere2 = std::make_shared<GameEntity>(sphereMesh, refractiveMat2);
	glassSphere2->GetTransform()->SetPosition(0, 1, -4);
	glassSphere2->GetTransform()->SetScale(0.7, 0.7, 0.7);

	std::shared_ptr<GameEntity> glassSphere3 = std::make_shared<GameEntity>(sphereMesh, refractiveMat3);
	glassSphere3->GetTransform()->SetPosition(-2, -0.3, -4);
	glassSphere3->GetTransform()->SetScale(0.7, 0.7, 0.7);

	std::shared_ptr<GameEntity> glassSphere4 = std::make_shared<GameEntity>(sphereMesh, refractiveMat4);
	glassSphere4->GetTransform()->SetPosition(1, -1, -4);
	glassSphere4->GetTransform()->SetScale(0.7, 0.7, 0.7);
	...
	entitiesLineup.push_back(glassWall);
	entitiesLineup.push_back(glassSphere);
	entitiesLineup.push_back(glassSphere2);
	entitiesLineup.push_back(glassSphere3);
	entitiesLineup.push_back(glassSphere4);
	...
	{
		// Load shaders
		texturePS = std::make_shared<SimplePixelShader>(Graphics::Device, Graphics::Context, FixPath(L"SimpleTexture.cso").c_str());
...
// --------------------------------------------------------
// Resize a RTVs and SRVs by creating new textures
// --------------------------------------------------------
void Game::ResizeAllRenderTargets()
{
	ResizeRenderTarget(colorRTV, colorSRV, DXGI_FORMAT_R8G8B8A8_UNORM);
	ResizeRenderTarget(silhouetteRTV, silhouetteSRV, DXGI_FORMAT_R8G8B8A8_UNORM);
}
	...
	DXGI_FORMAT colorFormat)
{
	...
	Microsoft::WRL::ComPtr<ID3D11Texture2D> texture;
	Graphics::Device->CreateTexture2D(&texDesc, 0, texture.GetAddressOf());

	// Release and create new RTV and SRV
	...
	Graphics::Device->CreateRenderTargetView(texture.Get(), &rtvDesc, rtv.ReleaseAndGetAddressOf());
	Graphics::Device->CreateShaderResourceView(texture.Get(), 0, srv.ReleaseAndGetAddressOf());
}
...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	// Change render target to render into the color texture
	Graphics::Context->OMSetRenderTargets(1, colorRTV.GetAddressOf(), Graphics::DepthBufferDSV.Get());

	// Reset list of refraction entities	
	refractionEntities.clear();
	...
	{
		// Save refraction entities for later
		if (e->GetMaterial()->IsRefractive())
		{
			refractionEntities.push_back(e);
		}
		else
		{
	...
	// Draw refractive objects to silhouette texture
	{
		...
		Graphics::Context->OMSetDepthStencilState(silhouetteDepthState.Get(), 0);
	...
	// Draw just the refractive objects 
	{
		...
		for (auto& e : refractionEntities)
		{
			...
			ps->SetFloat("refractionScale", e->GetMaterial()->GetRefractionScale());
